...
{
    private static readonly Func<LoggerConfiguration> _loggerConfigFunc = () => new LoggerConfiguration()
        ...
        .WriteTo.GrafanaLoki("http://loki:3100",
            ...
            labels: [new LokiLabel { Key = "service_name", Value = "NBomber" }]
        );
    ...
    public static void Main(string[] args)
    {
        ...
        var scenarioProps = new[]
        {
            CreateScenario("yarpproxy", "9000", 100, 1000),
            CreateScenario("ocelotgateway", "8000", 100, 1000),
        };
        ...
        NBomberRunner
            ...
            .WithLoggerConfig(_loggerConfigFunc)
            .Run();

        ScenarioProps CreateScenario(string name, string port, int increaseRate, int executingTime = 60)
        {
          var scenario = Scenario.Create($"{name} - rate:{increaseRate} - duration:{executingTime}", async context =>
            {
              var step1 = await Step.Run(
                $"Requesting [GET] 'hello-world' endpoint step for '{context.ScenarioInfo.ScenarioName}' scenario.",
                context, async () =>
                {
                  using HttpClient client = httpFactory.CreateClient();
                  var response = await client.GetAsync($"http://localhost:{port}/api/webapi01/hello-world");
                  return response.IsSuccessStatusCode
                    ? Response.Ok()
                    : Response.Fail();
                });

              var step2 = await Step.Run(
                $"Requesting [POST] 'req' endpoint step for '{context.ScenarioInfo.ScenarioName}' scenario.",
                context, async () =>
                {
                  using HttpClient client = httpFactory.CreateClient();
                  var json = new StringContent(content:
                    ...
                    """, Encoding.UTF8);
                  var response = await client.PostAsync($"http://localhost:{port}/api/webapi01/req", json);
                  return response.IsSuccessStatusCode
                    ? Response.Ok()
                    : Response.Fail();
                });
              
              return !step1.IsError && !step2.IsError ? Response.Ok() : Response.Fail();
            })
            .WithInit(context =>
            {
              context.Logger.Information($"Starting scenario '{context.ScenarioInfo.ScenarioName}'...");
              return Task.CompletedTask;
            })
            .WithClean(context =>
            {
              context.Logger.Information($"Finished scenario '{context.ScenarioInfo.ScenarioName}'...");
              return Task.CompletedTask;
            })
            .WithoutWarmUp()
            .WithLoadSimulations(
              Simulation.KeepConstant(increaseRate, TimeSpan.FromMinutes(2)),
              Simulation.RampingConstant(increaseRate * 15, TimeSpan.FromMinutes(5)),
              Simulation.RampingConstant(30, TimeSpan.FromMinutes(1)),
              Simulation.KeepConstant(increaseRate, TimeSpan.FromMinutes(2)),
              Simulation.RampingConstant(increaseRate * 15, TimeSpan.FromMinutes(10)));
                // .WithLoadSimulations(Simulation.RampingInject(rate: increaseRate,
                //     interval: TimeSpan.FromSeconds(10),
                //     during: TimeSpan.FromMinutes(5)),
                //   Simulation.InjectRandom(100, 1000, TimeSpan.FromSeconds(30), TimeSpan.FromMinutes(5))
                // );
            // .WithLoadSimulations(Simulation.IterationsForConstant(copies, executingTime));
